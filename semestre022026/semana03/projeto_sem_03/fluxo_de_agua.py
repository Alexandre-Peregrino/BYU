...
# Melhorias: 
# 1. Adição de variáveis DENSIDADE_AGUA, GRAVIDADE, VISCOSIDADE_DINAMICA, CONSTANTE_CONVERSAO_KPA. 
# 2. Criação da função converter_kPa_para_MCA.
# 3. Criação do test_converter_kPa_para_MCA para a função converter_kPa_para_MCA
...
GRAVIDADE = 9.80665
# Acelereção da gravidade na terra

VISCOSIDADE_DINAMICA = 0.0010016
# Viscosidade dinâmica da Água

CONSTANTE_CONVERSAO_KPA = 9.80665
...
def main():
    ...
    converter = float(input("Informe o valor em kPa: "))
    ...
    conversao = converter_kPa_para_MCA(converter)
    print(f"O valor em MCA é: {conversao:.2f}")
...
def calc_pressao_pela_altura(altura):
    return (DENSIDADE_AGUA * GRAVIDADE * altura) / 1000
...
def calc_num_reynolds(diametro_hidraulico, velocidade_fluido):
    return (DENSIDADE_AGUA * diametro_hidraulico * velocidade_fluido) / VISCOSIDADE_DINAMICA
...
def converter_kPa_para_MCA(kPa):
    return kPa/CONSTANTE_CONVERSAO_KPA
